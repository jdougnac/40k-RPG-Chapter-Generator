...
def demeanour():
    ...
    if Chapter.purity[1] ==2:
        Chapter.purity[1] = random.randint(0,1)
    if Chapter.purity[1] == 0:
        if Chapter.progenitor == "Ultramarines":
            demeanour = "Honour the Codex"
        elif Chapter.progenitor == "Blood Angels":
            demeanour = "The Red Thirst"
        elif Chapter.progenitor == "Dark Angels":
            demeanour = "Sons of the Lion"
        elif Chapter.progenitor == "Imperial Fists":
            demeanour = "Sons of Dorn"
        elif Chapter.progenitor == "White Scars":
            demeanour = "Son of the Steppes"
        elif Chapter.progenitor == "Raven Guard":
            demeanour = "Son of the Night"
        elif Chapter.progenitor == "Iron Hands":
            demeanour = "Iron and Hate"
        elif Chapter.progenitor == "Space Wolves":
            demeanour = "The Sons of Russ"
        elif Chapter.progenitor == "Salamanders":
            demeanour = "The Promethean Cult"
    elif Chapter.purity[1] == 1:
...
def deficiency():
    ...
    if deficiency_roll <= deficiency_chance:
        ...
        if specific_deficiency_roll <=90:
            Chapter.purity[2] = "Same as progenitor chapter\n"
        else:
            Chapter.purity[2] = 1
    if Chapter.purity[2] == 1:
        Chapter.purity[2] = ""
        deficiency_amount = 1
        missing_zygotes = 0
        ...
        inactive_list.sort()
        if inactive_list != []:
            Chapter.purity[2] += "Missing or Inactive Zygotes:\n"
            for item in inactive_list:
                Chapter.purity[2] +=item+"\n"
    deficiency = Chapter.purity[2]
    if deficiency == 0:
        deficiency = "None"
    return deficiency
...
def characteristic_modifier():
    ...
    characteristics = random.choice(characteristic_modifiers_list)
    return characteristics

#main function should determine how many figures there
#should be. Probably 30% chance of adding 1-3 extra figures
def figure_of_legend():
    figure = ""
...
def deed_of_legend():
    ...
    if deed_roll <=25:
        ...
        deed = "Led a campaign against rebels, brining a whole sector back to the Imperium"
    elif deed_roll <=85:
...
def home_world():
    ...
    return home_world_string
    
def chapter_organisation():
    ...
    if Chapter.progenitor == "Ultramarines":
        ...
        if chapter_organisation_roll <=5:
            ...
            chapter_organisation = "Unique Organisation"
    Chapter.organisation =chapter_organisation
    return chapter_organisation
...
def squad_mode_abilities():
    ...
    return squad_mode_abilities

def speciality_restrictions(): #should only occur if organisation is unique
...
def special_equipment(): #should only occur if organisation is Divergent or Unique
...
def chapter_enemies():
    ...
    if chapter_enemies_roll <=2:
        chapter_enemies = chapter_friends()
